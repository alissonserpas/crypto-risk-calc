...
// Dejamos solo las 3 principales como acceso rápido
const QUICK_COINS = [
...
export default function RiskCalculator() {
  ...
  // Nuevo estado para la búsqueda personalizada
  ...
  const fetchPrice = async (coinId: string) => {
    ...
    try {
      ...
      const response = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${cleanId}&vs_currencies=usd`);
      const data = await response.json();
      const price = data[cleanId]?.usd;
      
      if (price) {
        setInputs(prev => ({
          ...prev,
          entryPrice: price,
          stopLossPrice: Number((price * 0.99).toFixed(4)), // Ajustado a 4 decimales para monedas baratas
          takeProfitPrice: Number((price * 1.02).toFixed(4))
        }));
        setCustomCoin(''); // Limpiar el buscador si tuvo éxito
      } else {
        alert(`Price for "${cleanId}" not found. Try typing the full name (e.g. "ripple" instead of "xrp")`);
      }
    } catch (error) { 
      console.error(error); 
      alert("Connection error with CoinGecko API");
    } finally { 
      setIsLoading(false); 
    }
  };

  // Manejar la búsqueda al dar Enter
  const handleSearchSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    fetchPrice(customCoin);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 md:p-8 bg-slate-900 rounded-3xl shadow-2xl border border-slate-800">
      ...
      <div className="mb-8 text-center">
        ...
        <p className="text-slate-400">Calculate risk and reward with professional precision</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Account Balance</label>
            ...
            <div>
              <label className="text-xs font-bold text-slate-400 uppercase tracking-wider ml-1">Risk %</label>
              <div className="relative mt-1">
          ...
          {/* SECCIÓN DE PRECIOS MEJORADA (Buscador + Botones) */}
          <div className="bg-slate-950/50 p-4 rounded-xl border border-slate-800">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 block">Load Live Price</label>
            
            <div className="flex flex-col sm:flex-row gap-3">
              {/* 1. Botones Rápidos */}
              <div className="flex gap-2">
                {QUICK_COINS.map((coin) => (
                  <button key={coin.id} onClick={() => fetchPrice(coin.id)} disabled={isLoading} className="bg-slate-800 hover:bg-blue-600 border border-slate-700 hover:border-blue-500 text-white text-xs font-bold py-2 px-3 rounded-lg transition-all">
                    {coin.symbol}
                  </button>
                ))}
              </div>
              ...
              <form onSubmit={handleSearchSubmit} className="flex-grow flex gap-2">
                <input 
                  type="text" 
                  placeholder="Search coin (e.g. pepe, dogecoin)" 
                  value={customCoin}
                  onChange={(e) => setCustomCoin(e.target.value)}
                  className="w-full bg-slate-800 text-white text-sm rounded-lg px-3 focus:ring-2 focus:ring-blue-500 outline-none border border-slate-700 placeholder-slate-500"
                />
                <button 
                  type="submit"
                  disabled={isLoading || !customCoin}
                  className="bg-blue-600 hover:bg-blue-500 text-white px-4 rounded-lg font-bold text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {isLoading ? '...' : '🔍'}
                </button>
              </form>
          ...
          <div className="space-y-4 pt-2">
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              ...
              <div>
                <label className="text-xs text-blue-400 font-bold mb-1 block">ENTRY PRICE</label>
              ...
              <div>
                ...
                <input type="number" name="takeProfitPrice" value={inputs.takeProfitPrice} onChange={handleInputChange} className="w-full bg-slate-800 text-white rounded-lg py-2 px-3 focus:ring-2 focus:ring-emerald-500 outline-none border border-slate-700 font-mono text-sm" placeholder="Optional" />
              </div>
        ...
        <div className="lg:col-span-5 flex flex-col gap-4">
          ...
          <div className="bg-gradient-to-br from-slate-800 to-slate-900 rounded-2xl p-6 border border-slate-700 shadow-inner relative overflow-hidden">
            ...
            <span className="text-slate-400 text-xs font-bold uppercase tracking-wider">Position Size</span>
            ...
            <div className="mt-2 inline-block bg-slate-950/50 rounded px-2 py-1 text-xs text-slate-400 border border-slate-700">
              Total Value: ${results.positionValue}
            </div>
          </div>

          <div className="grid grid-cols-2 gap-4">
            <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
              <span className="text-red-400 text-xs font-bold uppercase">Risk (Loss)</span>
              <div className="text-xl font-bold text-white mt-1 font-mono">-${results.riskAmount}</div>
              <div className="text-xs text-slate-500 mt-1">{results.stopLossDistance}% distance</div>
            </div>

            <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
              <span className="text-emerald-400 text-xs font-bold uppercase">Reward (Profit)</span>
          ...
          <div className="flex-grow bg-blue-900/20 rounded-xl border border-blue-500/30 flex flex-col items-center justify-center p-4 min-h-[100px] text-center transition hover:bg-blue-900/30">
            <span className="text-blue-400 text-xs font-bold uppercase mb-1">Trading without discounts?</span>
            <a 
